...
const B2BCharts: React.FunctionComponent<Props> = ({ buildingId, meterType }) => {
	const [data, setData] = useState<ChartData<any>>(null);

	const getChartsData = async () => {
		const response = await fetch(
			`${SERVICE_API_URL}/data/house?buildingId=${buildingId}&startDate=01.05.2019&endDate=01.06.2019&measurementType=${meterType}`
		);
